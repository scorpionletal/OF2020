...
final class Router implements IRouter {
    ...
    */
    private $routerCollection = array(
        ...
        '/segundo' => null, # id por defecto
        '/tercero' => null # id por defecto
    );
    ...
    */
    final private function checkRequests()  {
        ...
        $this->routerCollection['/primero'] = array_key_exists(3, $this->real_request) ? $this->real_request[3] : null;
        $this->routerCollection['/segundo'] = array_key_exists(4, $this->real_request) ? $this->real_request[4] : null;
        $this->routerCollection['/tercero'] = array_key_exists(5, $this->real_request) ? $this->real_request[5] : null;
    }
    ...
    */
    final public function setRoute(string $index, string $rule = 'none')  {
        ...
        if (in_array($index, ['/controller', '/method', '/id', '/primero', '/segundo', '/tercero'])) {
     ...
     * Obtiene el primero
     * 
     * @return string con el primero.
     *           null si no está definido.
     */
    final public function getPrimero(bool $with_rules = false) {
        $primero = $this->routerCollection['/primero'];
        if ($with_rules && (!is_numeric($primero) || $primero <= 0)) {
            return null;
        }

        return $this->routerCollection['/primero'];
    }   
    /**
     * Obtiene el segundo
     * 
     * @return string con el segundo
     *           null si no está definido.
     */
    final public function getSegundo(bool $with_rules = false) {
        $segundo = $this->routerCollection['/segundo'];
        if ($with_rules && (!is_numeric($segundo) || $segundo <= 0)) {
            return null;
        }

        return $this->routerCollection['/segundo'];
    }   
        /**
     * Obtiene el método
     * 
     ...
     *           null si no está definido.
     */
    final public function getTercero(bool $with_rules = false) {
        $tercero = $this->routerCollection['/tercero'];
        if ($with_rules && (!is_numeric($tercero) || $tercero <= 0)) {
            return null;
        }

        return $this->routerCollection['/tercero'];
    }   
    /**
     * Obtiene el id
     *
